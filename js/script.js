$(function () {
  ...
  //     // 색상 보간 계산
  //     const startColor = { r: 165, g: 191, b: 140 }; // #A5BF8C
  //     const endColor = { r: 95, g: 91, b: 178 }; // #5F5BB2

  //     const r = Math.floor(startColor.r + (endColor.r - startColor.r) * scrollFraction); // R
  //     const g = Math.floor(startColor.g + (endColor.g - startColor.g) * scrollFraction); // G
  //     const b = Math.floor(startColor.b + (endColor.b - startColor.b) * scrollFraction); // B
  ...
  const quizData = [
    ...
    {
      question:
        "다음 중 보기에서 황단영이 졸업 전 학교에서 하고 싶어 했던 것은? (복수 정답)",
      options: [
        "비밀 아지트 만들기",
        "다같이 비빔밥 해먹기",
        "점심시간에 배달음식 시켜먹기",
    ...
    {
      question:
        "오합지졸들이 1학년 시절 김민서씨가 가장 많이 들고 다녔던 인형은? ",
      options: ["서하", "펭구", "식빵", "오리"],
      answer: ["식빵"],
    },
  ];
  ...
  function showResult() {
    ...
    if (score === totalQuestions) {
      ...
      $("#result .incorrect-questions")
        .text(incorrectQuestions.join(", "))
        .show(); // 틀린 문제 보이기
